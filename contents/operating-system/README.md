...
메인 메모리의 크기가 한정되어 있기때문에, 물리적인 메모리 크기보다 큰 프로세스를 실행시킬 수 없다. 그렇다면 메인 메모리보다 크기가 큰 프로세스를 실행시키고 싶으면 어떻게 해야 할까? 그래서 나온 방법이 바로 '가상메모리'이다.
<br/>
<br/>
...
2. 빈 페이지 프레임을 찾는다.
  1-1. 페이지 교체 알고리즘을 통해 희생될 페이지를 고른다.
  2-1. 희생될 페이지를 디스크에 기록하고, 관련 페이지 테이블을 수정한다.
