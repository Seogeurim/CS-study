...
- 스레드도 하나의 실행 흐름이므로 실행과 관련된 데이터가 필요하다.
  - 독립적 : 각 스레드는 자신만의 고유한 **스레드 ID, 프로그램 카운터, 레지스터 집합, 스택 영역**을 가진다.
  - 공유 : 그리고 **속한 프로세스 내의 코드/데이터/힙 영역과 기타 운영체제 자원**을 다른 스레드와 공유한다.
...
각 스레드가 독립적인 실행 흐름을 갖기위해서는 `독립적인 함수 호출이 보장`되어야 하기 때문입니다.
...
- **스레드** : 할당받은 자원을 이용한 프로세스의 실행 흐름의 단위이다.
...
스레드는 같은 프로세스 내에서 스택을 제외한 영역을 다른 스레드들과 공유한다. 이는 자원을 공유한다는 것인데, 이는 스레드간 `자원 동기화 관련 문제`를 유발한다. 
