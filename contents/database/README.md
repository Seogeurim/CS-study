...
반정규화는 진행하면 안됩니다. 개인정보에 관한 데이터베이스로서 데이터의 무결성과 보안이 제일 중요합니다. 따라서 반정규화가 아닌 데이터베이스 튜닝, 재구성이 필요해 보입니다.  
'삽입, 삭제, 수정 이상'이 일어나는 경우에는 데이터의 무결성이 깨질 수 있기에 정규화를 진행하였기 때문에 정규화가 불가피하고 성능저하가 일어나도 가져가야할 문제라고 생각합니다.  
